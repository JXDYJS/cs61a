...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        env.bindings[expressions.first.first] = do_lambda_form(Pair(expressions.first.rest,expressions.rest), env)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    a = LambdaProcedure(expressions.first,expressions.rest,env)
    return a
    # END PROBLEM 7
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            while clause.rest != nil:
                test = scheme_eval(clause.rest.first, env)
                clause = clause.rest
            return test
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    a = Frame(env)
    while bindings != nil:
        if bindings.first.first in a.bindings or (bindings.first.rest == nil or bindings.first.rest.rest != nil) or type(bindings.first.first) == int:
            raise SchemeError()
        a.bindings[bindings.first.first] = eval_all(bindings.first.rest,env)
