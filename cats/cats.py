...
def accuracy(typed, source):
    ...
    "*** YOUR CODE HERE ***"
    s=len(typed_words)
    if s==0 and len(source_words)==0:
        return 100.0
    elif s==0 or len(source_words)==0:
        return 0.0
    accuracy=0
    for i in range(s):
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed in source or source in typed:
        ...
        for i in range(min(len(source),len(typed))):
            if source[i]!=typed[i]:
                add=remove=change=typed
                add=add[:i]+source[i]+add[i:]
                if i!=len(typed)-1:
                    remove=remove[:i]+remove[i+1:]
                else:
                    remove=remove[:i]
                if i!=len(typed)-1:
                    change=change[:i]+source[i]+change[i+1:]
                else:
                    change=change[:i]+source[i]
                answer=min(minimum_mewtations(add,source,limit-1),
                           minimum_mewtations(remove,source,limit-1),
                           minimum_mewtations(change,source,limit-1))
                break
        return answer+1
...
def report_progress(typed, source, user_id, upload):
    ...
    lens=len(source)
    count = 0
    for i in range (min(lent,lens)):
        if typed[i] == source[i]:
            count += 1
        else:
            break
    d={'id':user_id,'progress':count/lens}
    upload(d)
    return count/lens
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(len(timestamps_per_player[i])-1):
            x.append(timestamps_per_player[i][j+1]-timestamps_per_player[i][j])
        time.append(x)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    minx=[[]for i in player_indices]
    for i in word_indices:
        mina=get_all_times(match)[0][i]
        mini=0
        for j in player_indices:
            if get_all_times(match)[j][i]<mina:
                mina=get_all_times(match)[j][i]
                mini=j
        minx[mini].append(get_all_words(match)[i])
    return minx
