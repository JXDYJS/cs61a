def berry_finder(t):
    ...
    def xx(t):
        if label(t) == "berry":
            ...
            return sum([xx(b) for b in branches(t)])
    a=xx(t)
    if a==0:
        return False
    else:
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    if label(t) == 'loki' and is_leaf(t):
        new_t=tree(lokis_replacement)
    else:
        new_t=tree(label(t))
    return new_t+[replace_loki_at_leaf(b, lokis_replacement) for b in branches(t)]
...
def dejavu(t, n):
    ...
    def xd(t,n):
        if label(t) == n and is_leaf(t):
            ...
            f=0
        return f+sum([xd(b, n-label(t)) for b in branches(t)])
    a=xd(t,n)
    if a:
        return True
    else:
...
def hailstone_tree(n, h):
    ...
    if (n-1)%3==0 and (n-1)/3!=1 and (n-1)/3%2==1:
        branches += [hailstone_tree(int((n-1)/3), h-1)]
