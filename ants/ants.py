...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if (self.is_container == False and place.ant.can_contain(self) == True) or (self.is_container == True and self.can_contain(place.ant) == True):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        t=self.place
        distance = 0
        while(t.entrance != None ):
            if t.bees != [] and self.lower_bound<= distance <=self.upper_bound:
                flag=1
                break
            t=t.entrance
            distance+=1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            i=0
            while(i<len(p.bees)):
                if p.bees[i].health > self.damage+1:
                   p.bees[i].reduce_health(self.damage+amount)
                   i+=1
                else:
                    p.bees[i].reduce_health(self.damage+amount)
            ...
            i=0
            while(i<len(p.bees)):
                if p.bees[i].health > 1:
                   p.bees[i].reduce_health(amount)
                   i+=1
                else:
                    p.bees[i].reduce_health(amount)
...
class TankAnt(ContainerAnt):
    ...
    def action(self,gamestate):
        ...
        p=self.place
        while(i<len(p.bees)):
            if (self.damage < p.bees[i].health ):
                p.bees[i].reduce_health(self.damage)
                i+=1
            else:
                p.bees[i].reduce_health(self.damage)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while(p.exit != None):
            p=p.exit
            if p.ant != None :
                if p.ant.is_container == True :
                    if p.ant not in self.already_double:
                        p.ant.double()
                        self.already_double.append(p.ant)
                    if p.ant.ant_contained != None and p.ant.ant_contained not in self.already_double:
                        p.ant.ant_contained.double()
                        self.already_double.append(p.ant.ant_contained)
                elif p.ant not in self.already_double:
                    p.ant.double()
                    self.already_double.append(p.ant)
